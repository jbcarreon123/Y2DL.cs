...
public class UpdateCommand : InteractionModuleBase<SocketInteractionContext>
{
    [SlashCommand("update", "Update info")]
    public async Task Update()
    {
        await DeferAsync(ephemeral: true);

        var youtubeService = Program.GetYoutubeService();
        ...
        try
        {
            ...
            if (Program.Config.Main.BotConfig.Status.Enabled)
            {
        ...
        {
            await Program.Log(new LogMessage(LogSeverity.Warning, "Channels", "An error occured",
                e));
        }

        await FollowupAsync("Channels updated.", ephemeral: true);
    }
}